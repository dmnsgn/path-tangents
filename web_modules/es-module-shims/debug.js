...
(function() {
    ...
    let featureDetectionPromise = Promise.resolve(dynamicImportCheck).then(()=>{
        ...
        if (!hasDocument) return Promise.all([
            ...
            wasmModulesEnabled && dynamicImport(createBlob(moduleCheck.replace("x", createBlob(new Uint8Array([
                0,
                97,
                115,
                109,
                ...
                0,
                0
            ]), "application/wasm")))).then(()=>supportsWasmModules = true, noop)
        ]);
    ...
    function getOrCreateLoad(url, fetchOpts, parent, source) {
        ...
        load.f = (async ()=>{
            if (!source) {
                ...
                if (t && !shimMode) {
                    if (t === "css" && !cssModulesEnabled || t === "json" && !jsonModulesEnabled || t === "wasm" && !wasmModulesEnabled) throw Error(`${t}-modules require <script type="esms-options">{ "polyfillEnable": ["${t}-modules"] }<${""}/script>`);
